...
class Tag extends BaseController {
    ...
    public function index() {
        $tag = new TagModel();
        $data = ['tags' => $tag->asObject()->paginate(10), 'pager' => $tag->pager];
    ...
    public function create() {
        ...
        if ($this->validate('tags')) {
            $id = $tag->insert(['name' => $this->request->getPost('name'), ]);
    ...
    public function edit($id = null) {
        ...
        $validation = \Config\Services::validation();
        $this->_loadDefaultView('Actualizar etiqueta', ['validation' => $validation, 'tag' => $tag->asObject()->find($id), ], 'edit');
    }
    ...
    public function update($id = null) {
        ...
        if ($this->validate('tags')) {
            $tag->update($id, ['name' => $this->request->getPost('name'), ]);
